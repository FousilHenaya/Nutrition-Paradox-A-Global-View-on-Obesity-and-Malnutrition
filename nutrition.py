...
st.sidebar.image("https://toppng.com/uploads/preview/hoto-herbalife-logo-11562868881f5ezuhfiyc.png", width=150)

st.sidebar.markdown(
    "<h1 style='font-size:40px;'>🍓HEALTH DATA</h1>", 
...
if st.session_state.show_eda:
    ...
    st.write("Shape of Obesity Data:", df_obesity.shape)
    st.write("First few rows of Obesity Data:")
    st.dataframe(df_obesity.head())
    
    st.write("Column Data Types:")
    st.write(df_obesity.dtypes)
    
    st.write("Summary Statistics:")
    st.write(df_obesity.describe())
    
    st.write("Missing Values in obesity Dataset:")
    ...
    st.write("Outliers (Negative Mean_Estimate):")
    st.write(df_obesity[df_obesity['Mean_Estimate'] < 0])
    
    st.write("Shape of malnutrition Data:", df_malnutrition.shape)
    st.write("First few rows of malnutrition Data:")
    st.dataframe(df_malnutrition.head())
    
    st.write("Column Data Types:")
    st.write(df_malnutrition.dtypes)
    
    st.write("Summary Statistics:")
    st.write(df_malnutrition.describe())
    ...
    st.write(df_malnutrition.duplicated().sum())
    
    st.write("Outliers (Negative Mean_Estimate):")
    st.write(df_malnutrition[df_malnutrition['Mean_Estimate'] < 0])
    
    st.subheader("📈 Distribution of Mean Estimate")

    fig, ax = plt.subplots(figsize=(8, 6))
    sns.histplot(df_obesity['Mean_Estimate'], kde=True, ax=ax)
    ax.set_title('Distribution of Mean Estimate (Obesity)')
    st.pyplot(fig)
    
    fig, ax = plt.subplots(figsize=(8, 6))
    sns.histplot(df_obesity['CI_Width'], kde=True, color='orange', ax=ax)
    ax.set_title('Distribution of CI Width (Obesity)')
    st.pyplot(fig)
    
    fig, ax = plt.subplots(figsize=(8, 6))
    sns.histplot(df_malnutrition['Mean_Estimate'], kde=True, ax=ax)
    ax.set_title('Distribution of Mean Estimate (Malnutrition)')
    st.pyplot(fig)
    
    fig, ax = plt.subplots(figsize=(8, 6))
    sns.histplot(df_malnutrition['CI_Width'], kde=True, color='orange', ax=ax)
    ax.set_title('Distribution of CI Width (Malnutrition)')
    st.pyplot(fig)

    st.subheader("📈 Trends of Mean Estimate Over Time by Region")

    fig, ax = plt.subplots(figsize=(10, 6))
    sns.lineplot(x='Year', y='Mean_Estimate', data=df_obesity, hue='Region', ax=ax)
    ax.set_title('Obesity Over Time by Region')
    st.pyplot(fig)
    
    fig, ax = plt.subplots(figsize=(10, 6))
    sns.lineplot(x='Year', y='Mean_Estimate', data=df_malnutrition, hue='Region', ax=ax)
    ax.set_title('Malnutrition Over Time by Region')
    st.pyplot(fig)
    
    st.subheader("📦 Box Plots by Region and Age Group")

    # Obesity by Region
    fig, ax = plt.subplots(figsize=(12, 6))
    sns.boxplot(x='Region', y='Mean_Estimate', data=df_obesity, ax=ax)
    ax.set_title('Obesity by Region')
    ...
    ax.set_title('Malnutrition by Region')
    ax.tick_params(axis='x', rotation=90)
    st.pyplot(fig)
    
    # Obesity by Age Group
    fig, ax = plt.subplots(figsize=(8, 6))
    sns.boxplot(x='age_group', y='Mean_Estimate', data=df_obesity, ax=ax)
    ax.set_title('Obesity by Age Group')
    st.pyplot(fig)
    
    # Malnutrition by Age Group
    fig, ax = plt.subplots(figsize=(8, 6))
    sns.boxplot(x='age_group', y='Mean_Estimate', data=df_malnutrition, ax=ax)
    ax.set_title('Malnutrition by Age Group')
    st.pyplot(fig)
    
    st.subheader("📊 Categorical Counts")

    fig, ax = plt.subplots(figsize=(10, 6))
    sns.countplot(x='Region', hue='obesity_level', data=df_obesity, ax=ax)
    ax.set_title('Obesity Levels by Region')
    ax.tick_params(axis='x', rotation=90)
    st.pyplot(fig)
    
    fig, ax = plt.subplots(figsize=(10, 6))
    sns.countplot(x='Region', hue='malnutrition_level', data=df_malnutrition, ax=ax)
    ax.set_title('Malnutrition Levels by Region')
    ax.tick_params(axis='x', rotation=90)
    st.pyplot(fig)
    ...
    fig, ax = plt.subplots(figsize=(8, 6))
    sns.heatmap(correlation_matrix, annot=True, cmap='coolwarm', cbar=True, ax=ax)
    ax.set_title('Obesity: Correlation Heatmap')
    st.pyplot(fig)
    
    correlation_matrix = df_malnutrition[['Mean_Estimate', 'CI_Width']].corr()
    fig, ax = plt.subplots(figsize=(8, 6))
    sns.heatmap(correlation_matrix, annot=True, cmap='coolwarm', cbar=True, ax=ax)
    ax.set_title('Malnutrition: Correlation Heatmap')
    st.pyplot(fig)
    
    st.subheader("🧠 Interactive Scatter Plots")

    fig = px.scatter(df_obesity, x='Mean_Estimate', y='CI_Width', color='Region',
                     title='Obesity: Mean Estimate vs CI Width')
    st.plotly_chart(fig)
    
    fig = px.scatter(df_malnutrition, x='Mean_Estimate', y='CI_Width', color='Region',
                     title='Malnutrition: Mean Estimate vs CI Width')
    st.plotly_chart(fig)
...
if query=="Top 5 regions with the highest average obesity levels in the most recent year(2022)":
    query = """
    ...
    # Convert result to a list of dicts or a pandas DataFrame for better display
    df = pd.DataFrame(result, columns=["Region", "Average Obesity Level"])

    # Display table nicely in Streamlit
    st.dataframe(df.style.format({"Average Obesity Level": "{:.2f}"}))

if query=="Top 5 countries with highest obesity estimates":
    query = """
    ...
    df = pd.DataFrame(result, columns=["Country", "Average Obesity Estimate"])
    
    # Display the DataFrame with formatting
    st.dataframe(df.style.format({"Average Obesity Estimate": "{:.2f}"}))

if query=="Average obesity by gender":
    query = """
...
if query=="Country count by obesity level category and age group":
    query = """
...
if query=="Top 5 countries least reliable countries(with highest CI_Width) and Top 5 most consistent countries (smallest average CI_Width)":
...
if query=="Average obesity by age group":
    query = """
...
if query=="Top 10 Countries with consistent low obesity (low average + low CI)over the years":
    query = """
        SELECT Country, 
...
if query=="Countries where female obesity exceeds male by large margin (same year)":
    query = """
        SELECT 
            o1.Country,
...
if query=="Global average obesity percentage per year":
    query = """
        SELECT 
            Year,
...
if query=="Avg. malnutrition by age group":
    query = """
        SELECT 
            age_group,
...
if query=="Top 5 countries with highest malnutrition(mean_estimate)":
    query = """
        SELECT 
            Country,
            AVG(Mean_Estimate) AS Avg_Malnutrition
...
if query=="Malnutrition trend in African region over the years":
    query = """
        SELECT 
            Year,
            AVG(Mean_Estimate) AS Avg_Malnutrition
...
if query=="Gender-based average malnutrition":
    query = """
        SELECT 
            Gender,
            AVG(Mean_Estimate) AS Avg_Malnutrition
...
if query=="Malnutrition level-wise (average CI_Width by age group)":
    query = """
        SELECT 
            malnutrition_level,
            age_group,
...
if query=="Yearly malnutrition change in specific countries(India, Nigeria, Brazil)":
    query = """
        SELECT 
            Year,
            Country,
...
if query=="Regions with lowest malnutrition averages":
    query = """
        SELECT 
            Region,
            AVG(Mean_Estimate) AS Avg_Malnutrition
...
if query=="Countries with increasing malnutrition":
    query = """
        SELECT 
            Country,
...
if query=="Min/Max malnutrition levels year-wise comparison":
    query = """
        SELECT 
            Year,
...
if query=="High CI_Width flags for monitoring(CI_width > 5)":
    query = """
...
if query=="Obesity vs malnutrition comparison by country":
    query = """
        SELECT 
            o.Country,
            o.Year,
...
if query=="Gender-based disparity in both obesity and malnutrition":
    query = """
        SELECT 
            o.Country,
            o.Gender,
...
if query=="Region-wise avg estimates side-by-side(Africa and America)":
    query = """
        SELECT 
            o.Region,
            o.Gender,
...
if query=="Countries with obesity up & malnutrition down":
   query = """
   SELECT 
       o.Country,
...
if query=="Age-wise trend analysis":
   # Query obesity trend
   curr.execute("""
   SELECT
       Year,
